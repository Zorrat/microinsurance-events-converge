...
import type { Config, EventSummary, GeminiRiskBand } from "../types";
...
export type GeminiRiskAssessmentResponse = {
...
const defaultGeminiAssessment = (config: Config): GeminiRiskAssessment => {
  return {
...
export const parseGeminiStructuredOutput = (rawText: string): GeminiRiskAssessmentResponse => {
  const parseJsonLoose = (input: string): unknown => {
    const trimmed = input.trim();
    try {
      return JSON.parse(trimmed);
    } catch {
      // Grounded responses can include markdown fences or lead/trail text.
...
type GeminiHttpInput = {
  url: string;
  payload: GeminiGenerateContentRequest;
  maxRetries: number;
};

const callGeminiGenerateContent = (
  runtime: Runtime<Config>,
  httpClient: HTTPClient,
  input: GeminiHttpInput,
): GeminiRiskAssessment => {
  const fetchGemini = httpClient.sendRequest(
    runtime,
    (sendRequester: HTTPSendRequester, req: GeminiHttpInput): GeminiRiskAssessment => {
    ...
    consensusIdenticalAggregation<GeminiRiskAssessment>(),
  );

  return fetchGemini(input).result();
};

export const assessGeminiRisk = (
  runtime: Runtime<Config>,
  httpClient: HTTPClient,
  event: EventSummary,
  config: Config,
  ...
  const apiKey = getSecretValue(runtime, apiSecretName, config);
  ...
  return callGeminiGenerateContent(runtime, httpClient, {
    url,
    payload,
    maxRetries,
  });
...
export const assessGeminiRiskStub = (_event: EventSummary, config: Config): GeminiRiskAssessment => {
