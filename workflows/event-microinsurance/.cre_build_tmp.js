...
var init_transaction = __esm(() => {
  init_base();
  InvalidLegacyVError = class InvalidLegacyVError2 extends BaseError {
    constructor({ v }) {
  ...
  InvalidSerializableTransactionError = class InvalidSerializableTransactionError2 extends BaseError {
  ...
  InvalidStorageKeySizeError = class InvalidStorageKeySizeError2 extends BaseError {
...
var init_node = __esm(() => {
  init_formatGwei();
  init_base();
  ExecutionRevertedError = class ExecutionRevertedError2 extends BaseError {
  ...
  FeeCapTooHighError = class FeeCapTooHighError2 extends BaseError {
  ...
  FeeCapTooLowError = class FeeCapTooLowError2 extends BaseError {
    constructor({ cause, maxFeePerGas } = {}) {
  ...
  NonceTooHighError = class NonceTooHighError2 extends BaseError {
  ...
  NonceTooLowError = class NonceTooLowError2 extends BaseError {
    constructor({ cause, nonce } = {}) {
      super([
...
`), { cause, name: "NonceTooLowError" });
  ...
  NonceMaxValueError = class NonceMaxValueError2 extends BaseError {
    constructor({ cause, nonce } = {}) {
  ...
  InsufficientFundsError = class InsufficientFundsError2 extends BaseError {
    constructor({ cause } = {}) {
      super([
...
`), {
  ...
  IntrinsicGasTooHighError = class IntrinsicGasTooHighError2 extends BaseError {
  ...
  IntrinsicGasTooLowError = class IntrinsicGasTooLowError2 extends BaseError {
    constructor({ cause, gas } = {}) {
  ...
  TransactionTypeNotSupportedError = class TransactionTypeNotSupportedError2 extends BaseError {
    constructor({ cause }) {
  ...
  TipAboveFeeCapError = class TipAboveFeeCapError2 extends BaseError {
...
var init_chain = __esm(() => {
  init_base();
  InvalidChainIdError = class InvalidChainIdError2 extends BaseError {
    constructor({ chainId }) {
...
var mustHexAddress = (value2, label) => {
  if (!isAddress(value2))
    throw new Error(`${label} is not a valid EVM address`);
  return value2;
};
var nowSec = (runtime2) => {
  return Math.floor(runtime2.now().getTime() / 1000);
};
var toUint64 = (value2) => {
  const v = BigInt(value2);
  if (v < 0n || v > 18446744073709551615n) {
    throw new Error(`uint64 overflow: ${v.toString()}`);
  }
  return v;
};
var toUint128 = (value2) => {
  const v = BigInt(value2);
  if (v < 0n || v > 340282366920938463463374607431768211455n) {
    throw new Error(`uint128 overflow: ${v.toString()}`);
  }
  return v;
};
var parseTimestampSec = (value2) => {
  if (typeof value2 === "number" && Number.isFinite(value2)) {
    if (value2 > 1000000000000)
      return Math.floor(value2 / 1000);
    return Math.floor(value2);
  }
  if (typeof value2 === "string") {
    const asNum = Number(value2);
    if (Number.isFinite(asNum)) {
      if (asNum > 1000000000000)
        return Math.floor(asNum / 1000);
      return Math.floor(asNum);
    }
    const asDate = Date.parse(value2);
    if (!Number.isNaN(asDate))
      return Math.floor(asDate / 1000);
  }
  return;
};
var parseEventbriteEventIdFromUrl = (rawUrl) => {
  ...
  const isEventbriteHost = host === "eventbrite.com" || host.endsWith(".eventbrite.com") || /^eventbrite\.[a-z]{2,}(?:\.[a-z]{2,})?$/.test(host) || hostParts.some((part, idx) => part === "eventbrite" && idx < hostParts.length - 1);
  if (!isEventbriteHost)
    throw new Error("INVALID_EVENTBRITE_URL");
  const decodeSafe = (value2) => {
    try {
  ...
  if (query.length > 0) {
    for (const pair of query.split("&")) {
      ...
      if (k === "eid" || k === "event_id" || k === "eventId")
        queryCandidates.push(v);
    }
  }
...
var normalizeEventbriteEvent = (payload) => {
  ...
  const eventStart = parseTimestampSec(event?.start?.utc) ?? parseTimestampSec(event?.start?.local) ?? parseTimestampSec(event?.start_at) ?? parseTimestampSec(event?.start_time);
  const eventEnd = parseTimestampSec(event?.end?.utc) ?? parseTimestampSec(event?.end?.local) ?? parseTimestampSec(event?.end_at) ?? parseTimestampSec(event?.end_time);
  const normalizedStatus = typeof rawStatus === "string" ? rawStatus.toLowerCase() : "";
  const canceled = normalizedStatus === "canceled" || normalizedStatus === "cancelled" || Boolean(event?.canceled_at) || Boolean(event?.cancelled_at);
  const parsedCapacity = typeof event?.capacity === "number" ? event.capacity : typeof event?.capacity === "string" && Number.isFinite(Number(event.capacity)) ? Number(event.capacity) : undefined;
  const salesStatus = typeof event?.event_sales_status?.sales_status === "string" ? event.event_sales_status.sales_status : typeof event?.sales_status === "string" ? event.sales_status : undefined;
  ...
  if (eventUrl !== undefined)
    summary.eventUrl = eventUrl;
  if (parsedCapacity !== undefined)
    summary.capacity = parsedCapacity;
  if (onlineEvent !== undefined)
    summary.onlineEvent = onlineEvent;
  if (salesStatus !== undefined)
    summary.salesStatus = salesStatus;
  if (eventStart !== undefined)
    summary.eventStart = eventStart;
...
var handleClaim = async (runtime2, httpClient, evm, input, config) => {
  try {
    ...
    const receiverRead = evm.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: receiver,
    ...
    const policyRead = evm.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: policyNft,
        data: policyReadData
    ...
    if (event.canceled === true)
      decision = "PAY";
    else if (eventOverBoundarySec !== undefined && tNow > eventOverBoundarySec)
      decision = "RESOLVE_NO_PAYOUT";
    if (decision === "NO_OP") {
      return {
        ok: true,
    ...
    const reportBytes = decision === "PAY" ? encodeReportBytes({ action: 1, policyId, mint: emptyMintData() }) : encodeReportBytes({ action: 2, policyId, mint: emptyMintData() });
    const report2 = runtime2.report(prepareReportRequest(reportBytes)).result();
    const write2 = evm.writeReport(runtime2, {
      receiver,
      report: report2,
      gasConfig: { gasLimit: "800000" }
    }).result();
    const txHash = write2.txHash ? bytesToHex(write2.txHash) : undefined;
    runtime2.log(`CLAIM decision=${decision} txHash=${txHash ?? "n/a"}`);
    return {
      ok: true,
      action: "CLAIM",
      decision,
...
var hashEventId = (eventId) => {
  return keccak256(encodePacked(["string"], [eventId]));
};
var normalizeNonce = (nonce) => {
  if (!/^0x[0-9a-fA-F]{64}$/.test(nonce)) {
    throw new Error("nonce must be 32-byte hex");
  }
  return nonce;
};
var randomNonceHex = (runtime2) => {
  const t = BigInt(nowSec(runtime2));
  const jitter = BigInt(Math.floor(Math.random() * 1e9));
  return keccak256(encodePacked(["uint256", "uint256"], [t, jitter]));
};
var computeQuoteHash = (quote) => {
  if (!isAddress(quote.insured))
    throw new Error("quote.insured invalid");
  if (!/^0x[0-9a-fA-F]{64}$/.test(quote.eventIdHash))
    throw new Error("quote.eventIdHash invalid");
  if (!/^0x[0-9a-fA-F]{64}$/.test(quote.nonce))
    throw new Error("quote.nonce invalid");
...
var getQuoteSigner = (runtime2, config) => {
  const pk = getSecretValue(runtime2, config.quoteSignerPrivateKeySecretName, config);
  if (!/^0x[0-9a-fA-F]{64}$/.test(pk)) {
    throw new Error("quote signer private key must be 32-byte hex");
  }
  return privateKeyToAccount(pk);
};
var signQuote = async (runtime2, quote, config) => {
  const account = getQuoteSigner(runtime2, config);
  const quoteHash = computeQuoteHash(quote);
  const signature = await account.signMessage({ message: { raw: hexToBytes2(quoteHash) } });
  if (config.quoteSignerAddress && account.address.toLowerCase() !== config.quoteSignerAddress.toLowerCase()) {
...
var verifySignedQuote = async (runtime2, signedQuote, config) => {
  ...
  const recovered = await recoverMessageAddress({
    message: { raw: hexToBytes2(signedQuote.quoteHash) },
    signature: signedQuote.signature
  });
  const expectedSigner = getQuoteSigner(runtime2, config).address;
  if (config.quoteSignerAddress && expectedSigner.toLowerCase() !== config.quoteSignerAddress.toLowerCase()) {
    throw new Error("Configured quoteSignerAddress does not match signer private key");
  }
  if (recovered.toLowerCase() !== expectedSigner.toLowerCase()) {
    throw new Error("BAD_QUOTE_SIGNATURE");
  }
  if (signedQuote.quote.quoteExpiry <= nowSec(runtime2)) {
    throw new Error("QUOTE_EXPIRED");
  }
  return { quote: signedQuote.quote, signer: recovered };
};
var handleMint = async (runtime2, httpClient, evm, input, config) => {
  try {
    if (!input.approved)
      return { ok: false, error: "NOT_APPROVED" };
    const receiver = mustHexAddress(config.receiver, "receiver");
    const { quote } = await verifySignedQuote(runtime2, input.signedQuote, config);
    const event = fetchEventbriteEvent(runtime2, httpClient, quote.eventId, config);
    if (event.canceled === true)
      return { ok: false, error: "EVENT_ALREADY_CANCELED" };
    if (quote.quoteExpiry <= nowSec(runtime2))
      return { ok: false, error: "QUOTE_EXPIRED" };
    const readAndDecode = (to, abi, functionName, args = []) => {
      const callData = encodeFunctionData({
        abi,
        functionName,
        args
      });
      const read2 = evm.callContract(runtime2, {
        call: encodeCallMsg({
          from: zeroAddress,
          to,
          data: callData
      ...
      const dataHex = bytesToHex(read2.data);
      if (dataHex === "0x")
        throw new Error(`EVM_EMPTY_RESPONSE:${functionName}`);
    ...
    const txHash = write2.txHash ? bytesToHex(write2.txHash) : undefined;
    runtime2.log(`MINT submitted txHash=${txHash ?? "n/a"}`);
    ...
    if (nextPolicyIdAfter !== nextPolicyIdBefore + 1n) {
      runtime2.log(`MINT nextPolicyId advanced unexpectedly before=${nextPolicyIdBefore.toString()} after=${nextPolicyIdAfter.toString()}`);
    }
...
var handleQuoteCheck = async (runtime2, httpClient, evm, input, config) => {
  try {
    ...
    const normalizedForParse = /^[a-zA-Z][a-zA-Z0-9+.-]*:\/\//.test(rawEventUrl) ? rawEventUrl : `https://${rawEventUrl}`;
    runtime2.log(`[QUOTE_CHECK] eventUrl.normalized=${normalizedForParse}`);
    const digitCandidates = rawEventUrl.match(/[0-9]{6,}/g) ?? [];
    runtime2.log(`[QUOTE_CHECK] eventUrl.digitCandidates=${digitCandidates.join(",") || "(none)"}`);
    let canonicalEventId;
    try {
      canonicalEventId = parseEventbriteEventIdFromUrl(rawEventUrl);
      runtime2.log(`[QUOTE_CHECK] eventId.parsed=${canonicalEventId}`);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      runtime2.log(`[QUOTE_CHECK] eventId.parse-failed reason=${msg}`);
      throw e;
    }
    const readContractHex = (label, to, data) => {
      const read2 = evm.callContract(runtime2, {
        call: encodeCallMsg({
          from: zeroAddress,
          to,
      ...
      const dataHex = bytesToHex(read2.data);
      runtime2.log(`[QUOTE_CHECK] evm.call label=${label} from=${zeroAddress} to=${to} data=${dataHex}`);
      if (dataHex === "0x") {
        throw new Error(`EVM_EMPTY_RESPONSE:${label} (verify receiver/contract address and chainSelectorName)`);
      }
      return dataHex;
    };
    const readAndDecode = (label, to, abi, functionName, args = []) => {
      const callData = encodeFunctionData({
        abi,
        functionName,
        args
      ...
      runtime2.log(`[QUOTE_CHECK] evm.read label=${label} to=${to} fn=${functionName} data=${dataHex}`);
      try {
        return decodeFunctionResult({
          abi,
          functionName,
    ...
    const requiredReserves = readAndDecode("vault.requiredReserves", policyVault, PolicyVaultABI, "requiredReserves");
    const totalActiveLiabilityUSDC = readAndDecode("vault.totalActiveLiabilityUSDC", policyVault, PolicyVaultABI, "totalActiveLiabilityUSDC");
    const minReserveRatioBps = readAndDecode("vault.minReserveRatioBps", policyVault, PolicyVaultABI, "minReserveRatioBps");
...
`)[0] : "";
      runtime2.log(`[QUOTE_CHECK] error message=${msg}${stackTop ? ` stack=${stackTop}` : ""}`);
    } catch {}
    ...
    handler(trigger, async (runtime2, payload) => {
      try {
        if (config.authorizedKeys.length === 0) {
          runtime2.log("authorizedKeys empty: simulation-only; deployment requires configured keys");
        }
        ...
        if (!network248)
          return { ok: false, error: "Network not found in getNetwork()" };
        const httpClient = new ClientCapability2;
        const evm = new ClientCapability(network248.chainSelector.selector);
        if (input.action === "QUOTE_CHECK")
          return handleQuoteCheck(runtime2, httpClient, evm, input, config);
        if (input.action === "MINT")
          return handleMint(runtime2, httpClient, evm, input, config);
        if (input.action === "CLAIM")
          return handleClaim(runtime2, httpClient, evm, input, config);
